...
if [ -n "${GATE_VALWEBHOOK}" ]; then
  ...
  "${root}/bin/ck8s" ops kubectl wc delete "${GATE_VALWEBHOOK}"
fi

# Destroy user subnamespaces before their parent namespaces,
# this might fail if there are multiple levels.
"${root}/bin/ck8s" ops kubectl wc delete subns --all-namespaces --all
...
"${root}/bin/ck8s" ops helmfile wc -l app=dev-rbac destroy

# Might fail to be destroyed the first time, therefore run it once now
"${root}/bin/ck8s" ops helmfile wc -l app=hnc destroy

# Makes sure to uninstall Velero properly: https://velero.io/docs/v1.13/uninstalling/
"${root}/bin/ck8s" ops velero wc uninstall

# Destroy all helm releases
"${root}/bin/ck8s" ops helmfile wc -l app!=cert-manager -l app!=admin-rbac destroy

# Clean up namespaces and any other resources left behind by the apps
"${root}/bin/ck8s" ops kubectl wc delete ns alertmanager falco fluentd-system fluentd gatekeeper-system gpu-operator hnc-system ingress-nginx monitoring velero kured

# Clean up any leftover challenges
mapfile -t CHALLENGES < <(
  "${root}/bin/ck8s" ops kubectl wc get challenge --all-namespaces -oyaml |
...
if [ "${clusterAPIEnabled}" = "false" ]; then
  ...
  "${root}/bin/ck8s" ops helmfile wc -l app=cert-manager destroy
else
...
"${root}/bin/ck8s" ops kubectl wc delete pvc --all-namespaces --all

# Velero-specific removal: https://velero.io/docs/v1.10/uninstalling/
"${root}/bin/ck8s" ops kubectl wc delete crds -l component=velero

if [ "${clusterAPIEnabled}" = "false" ]; then
  # Cert-manager specific removal
  "${root}/bin/ck8s" ops kubectl wc delete namespace cert-manager
  #"${root}/bin/ck8s" ops kubectl wc delete crds -l app.kubernetes.io/name=cert-manager
else
  log_info "Cluster API provisioned cluster, skipping deletion of cert-manager namespace"
fi

# Delete kube prometheus stack CRDs
mapfile -t PROM_CRDS < <("${root}/bin/ck8s" ops kubectl wc api-resources --api-group=monitoring.coreos.com -o name)
if [[ "${#PROM_CRDS[@]}" -gt 0 ]]; then
  "${root}/bin/ck8s" ops kubectl wc delete crds "${PROM_CRDS[@]}"
fi

# Delete Trivy CRDs
mapfile -t TRIVY_CRDS < <("${root}/bin/ck8s" ops kubectl wc api-resources --api-group=aquasecurity.github.io -o name)
if [[ "${#TRIVY_CRDS[@]}" -gt 0 ]]; then
  "${root}/bin/ck8s" ops kubectl wc delete crds "${TRIVY_CRDS[@]}"
fi

# Delete Gatekeeper CRDs
GATE_CRDS=$("${root}/bin/ck8s" ops kubectl wc get crds -l gatekeeper.sh/system=yes -oname)
if [ -n "$GATE_CRDS" ]; then
  # shellcheck disable=SC2086
  "${root}/bin/ck8s" ops kubectl wc delete --ignore-not-found=true $GATE_CRDS
fi

GATE_CONS=$("${root}/bin/ck8s" ops kubectl wc get crds -l gatekeeper.sh/constraint=yes -oname)
if [ -n "$GATE_CONS" ]; then
  # shellcheck disable=SC2086
  "${root}/bin/ck8s" ops kubectl wc delete --ignore-not-found=true $GATE_CONS
fi

# Delete HNC CRDs
mapfile -t HNC_CRDS < <("${root}/bin/ck8s" ops kubectl wc api-resources --api-group=hnc.x-k8s.io -o name)
if [[ "${#HNC_CRDS[@]}" -gt 0 ]]; then
  "${root}/bin/ck8s" ops kubectl wc delete crds "${HNC_CRDS[@]}"
fi

"${root}/bin/ck8s" ops helmfile wc -l app=admin-rbac destroy

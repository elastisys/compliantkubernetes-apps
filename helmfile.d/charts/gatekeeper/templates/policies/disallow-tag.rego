...
violation[{"msg": msg}] {
    ...
    any(tags)
    msg := sprintf("The container named <%v> uses the :latest tag. Welkin® requires all images to have explicit tags. Read more at https://elastisys.io/welkin/user-guide/safeguards/enforce-no-latest-tag/", [container.name])
}
...
violation[{"msg": msg}] {
    ...
    not all(tag)
    msg := sprintf("The container named <%v> didn't specify an image tag. Welkin® requires all images to have tags to avoid the implicit :latest tag. Read more at https://elastisys.io/welkin/user-guide/safeguards/enforce-no-latest-tag/", [container.name])
}

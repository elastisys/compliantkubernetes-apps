...
resolve() {
  ...
  create)
    ...
    if [[ -n "${CI:-}" ]]; then
      ...
      elif command -v unbound-control || test -x /usr/sbin/unbound-control; then
  ...
  delete)
    if [[ -n "${CI:-}" ]]; then
      ...
      elif command -v unbound-control || test -x /usr/sbin/unbound-control && log.continue "remove forward for ${domain}?"; then

...
setup_file() {
  ...
  export provider="safespring"
  export installer="kubespray"
...
for release in "${releases[@]}"; do
  ...
  for cluster in sc wc; do
    bats_test_function --description "template is consistent - safespring kubespray ${cluster} ${release}" --tags resources -- template_test "${cluster}" "${release}"
  done
done

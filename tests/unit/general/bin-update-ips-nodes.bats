...
@test "ck8s update-ips wc apply --enable nodes" {
  ...
  assert_equal "$(yq.dig wc '.networkPolicies.global.wcNodes.ips | . style="flow"')" \
...
@test "ck8s update-ips both apply --enable nodes" {
  ...
  run ck8s update-ips both apply --enable nodes

  assert_equal "$(yq.dig sc '.networkPolicies.global.scNodes.ips | . style="flow"')" \
...
@test "ck8s update-ips both apply --enable nodes --ipv6" {
  mock_set_output "${mock_kubectl}" "192.0.2.1" 1              # node internal
  mock_set_output "${mock_kubectl}" "198.51.100.1" 2           # calico ipip
  mock_set_output "${mock_kubectl}" "203.0.113.1" 3            # calico vxlan
  mock_set_output "${mock_kubectl}" "203.0.113.101" 4          # calico wireguard
  ...
  run ck8s update-ips both apply --enable nodes --ipv6

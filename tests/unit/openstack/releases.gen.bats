...
# bats file_tags=static,releases,openstack
...
setup_file() {
  ...
  env.init openstack kubespray prod
...
@test "releases have constraint templates through needs - openstack - service cluster" {
...
@test "releases have constraint templates through needs - openstack - workload cluster" {
  # used filter: a constraint's kind...
  # created filter: should exist in a constraint's templates spec
...
@test "releases have custom resource definitions through needs - openstack - service cluster" {
...
@test "releases have custom resource definitions through needs - openstack - workload cluster" {
  # used filter: a custom resource's kind...
  # created filter: should exist in a custom resource definition's templates spec
  releases_have_through_needs wc \
    'select(.apiVersion | test("^(.+\.k8s\.io/|apps/|batch/|policy/|constraints.gatekeeper.sh/|)v1.*") | not) | .kind | "CustomResourceDefinition/" + .' \
...
@test "releases have namespaces through needs - openstack - service cluster" {
...
@test "releases have namespaces through needs - openstack - workload cluster" {
  # used filter: a resource's namespace (except certain defaults)...
  # created filter: should exist
  releases_have_through_needs wc \
    '.metadata.namespace | select(. != "default" and . != "kube-system" and . != "rook-ceph") | "Namespace/" + .' \
...
@test "releases with custom resources have validation on install disabled - openstack - service cluster" {
  release_with_custom_resources_have_validation_on_install_disabled sc
}

@test "releases with custom resources have validation on install disabled - openstack - workload cluster" {

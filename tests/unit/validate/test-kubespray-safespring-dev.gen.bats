...
setup_file() {
  ...
  env.init dev safespring kubespray
}
...
@test "configuration is valid - kubespray:safespring:dev - service cluster" {
  run ck8s validate sc <<< $'y\n'

  assert_success
}

@test "configuration is valid - kubespray:safespring:dev - workload cluster" {
  run ck8s validate wc <<< $'y\n'

  assert_success
}

@test "configuration is invalid - kubespray:safespring:dev - service cluster" {
...
@test "configuration is invalid - kubespray:safespring:dev - workload cluster" {
...
@test "manifests are valid - kubespray:safespring:dev - service cluster" {
  run helmfile_template_kubeconform service_cluster

  assert_success
}

@test "manifests are valid - kubespray:safespring:dev - workload cluster" {

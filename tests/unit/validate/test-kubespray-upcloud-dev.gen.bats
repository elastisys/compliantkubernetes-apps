...
setup_file() {
  ...
  env.init dev upcloud kubespray
}
...
@test "configuration is valid - kubespray:upcloud:dev - service cluster" {
  run ck8s validate sc <<< $'y\n'

  assert_success
}

@test "configuration is valid - kubespray:upcloud:dev - workload cluster" {
  run ck8s validate wc <<< $'y\n'

  assert_success
}

@test "configuration is invalid - kubespray:upcloud:dev - service cluster" {
...
@test "configuration is invalid - kubespray:upcloud:dev - workload cluster" {
...
@test "manifests are valid - kubespray:upcloud:dev - service cluster" {
  run helmfile_template_kubeconform service_cluster

  assert_success
}

@test "manifests are valid - kubespray:upcloud:dev - workload cluster" {

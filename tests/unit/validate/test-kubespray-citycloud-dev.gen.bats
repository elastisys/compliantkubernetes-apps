...
setup_file() {
  ...
  env.init dev citycloud kubespray
}
...
@test "configuration is valid - kubespray:citycloud:dev - service cluster" {
  run ck8s validate sc <<< $'y\n'

  assert_success
}

@test "configuration is valid - kubespray:citycloud:dev - workload cluster" {
  run ck8s validate wc <<< $'y\n'

  assert_success
}

@test "configuration is invalid - kubespray:citycloud:dev - service cluster" {
...
@test "configuration is invalid - kubespray:citycloud:dev - workload cluster" {
...
@test "manifests are valid - kubespray:citycloud:dev - service cluster" {
  run helmfile_template_kubeconform service_cluster

  assert_success
}

@test "manifests are valid - kubespray:citycloud:dev - workload cluster" {

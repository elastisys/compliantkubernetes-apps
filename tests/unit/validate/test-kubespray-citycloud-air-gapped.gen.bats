...
@test "configuration is valid - kubespray:citycloud:air-gapped - service cluster" {
  run ck8s validate sc <<< $'y\n'

  assert_success
}

@test "configuration is valid - kubespray:citycloud:air-gapped - workload cluster" {
  run ck8s validate wc <<< $'y\n'

  assert_success
}

@test "configuration is invalid - kubespray:citycloud:air-gapped - service cluster" {
...
@test "configuration is invalid - kubespray:citycloud:air-gapped - workload cluster" {
...
@test "manifests are valid - kubespray:citycloud:air-gapped - service cluster" {
  run helmfile_template_kubeconform service_cluster

  assert_success
}

@test "manifests are valid - kubespray:citycloud:air-gapped - workload cluster" {

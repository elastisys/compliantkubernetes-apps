...
@test "configuration is valid - kubespray:elastx:dev - service cluster" {
  run ck8s validate sc <<< $'y\n'

  assert_success
}

@test "configuration is valid - kubespray:elastx:dev - workload cluster" {
  run ck8s validate wc <<< $'y\n'

  assert_success
}

@test "configuration is invalid - kubespray:elastx:dev - service cluster" {
...
@test "configuration is invalid - kubespray:elastx:dev - workload cluster" {
...
@test "manifests are valid - kubespray:elastx:dev - service cluster" {
  run helmfile_template_kubeconform service_cluster

  assert_success
}

@test "manifests are valid - kubespray:elastx:dev - workload cluster" {

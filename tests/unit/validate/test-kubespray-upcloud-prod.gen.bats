...
@test "configuration is valid - kubespray:upcloud:prod - service cluster" {
  run ck8s validate sc <<< $'y\n'

  assert_success
}

@test "configuration is valid - kubespray:upcloud:prod - workload cluster" {
  run ck8s validate wc <<< $'y\n'

  assert_success
}

@test "configuration is invalid - kubespray:upcloud:prod - service cluster" {
...
@test "configuration is invalid - kubespray:upcloud:prod - workload cluster" {
...
@test "manifests are valid - kubespray:upcloud:prod - service cluster" {
  run helmfile_template_kubeconform service_cluster

  assert_success
}

@test "manifests are valid - kubespray:upcloud:prod - workload cluster" {

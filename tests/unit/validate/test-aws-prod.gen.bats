...
@test "configuration is valid - aws:prod - service cluster" {
  run ck8s validate sc <<< $'y\n'

  assert_success
}

@test "configuration is valid - aws:prod - workload cluster" {
  run ck8s validate wc <<< $'y\n'

  assert_success
}

@test "configuration is invalid - aws:prod - service cluster" {
...
@test "configuration is invalid - aws:prod - workload cluster" {
...
@test "manifests are valid - aws:prod - service cluster" {
  run helmfile_template_kubeconform service_cluster

  assert_success
}

@test "manifests are valid - aws:prod - workload cluster" {

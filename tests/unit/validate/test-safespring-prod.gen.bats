...
@test "configuration is valid - safespring:prod - service cluster" {
  run ck8s validate sc <<< $'y\n'

  assert_success
}

@test "configuration is valid - safespring:prod - workload cluster" {
...
@test "manifests are valid - safespring:prod - service cluster" {
  run helmfile_template_kubeconform service_cluster

  assert_success
}

@test "manifests are valid - safespring:prod - workload cluster" {

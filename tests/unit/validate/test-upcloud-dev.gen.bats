...
@test "configuration is valid - upcloud:dev - service cluster" {
  run ck8s validate sc <<< $'y\n'

  assert_success
}

@test "configuration is valid - upcloud:dev - workload cluster" {
  run ck8s validate wc <<< $'y\n'

  assert_success
}

@test "configuration is invalid - upcloud:dev - service cluster" {
...
@test "configuration is invalid - upcloud:dev - workload cluster" {
...
@test "manifests are valid - upcloud:dev - service cluster" {
  run helmfile_template_kubeconform service_cluster

  assert_success
}

@test "manifests are valid - upcloud:dev - workload cluster" {

...
setup_file() {
  ...
  env.init air-gapped baremetal capi
}
...
@test "configuration is valid - capi:baremetal:air-gapped - service cluster" {
  run ck8s validate sc <<< $'y\n'

  assert_success
}

@test "configuration is valid - capi:baremetal:air-gapped - workload cluster" {
  run ck8s validate wc <<< $'y\n'

  assert_success
}

@test "configuration is invalid - capi:baremetal:air-gapped - service cluster" {
...
@test "configuration is invalid - capi:baremetal:air-gapped - workload cluster" {
...
@test "manifests are valid - capi:baremetal:air-gapped - service cluster" {
  run helmfile_template_kubeconform service_cluster

  assert_success
}

@test "manifests are valid - capi:baremetal:air-gapped - workload cluster" {

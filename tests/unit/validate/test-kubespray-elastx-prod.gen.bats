...
setup_file() {
  ...
  env.init prod elastx kubespray
}
...
@test "configuration is valid - kubespray:elastx:prod - service cluster" {
  run ck8s validate sc <<< $'y\n'

  assert_success
}

@test "configuration is valid - kubespray:elastx:prod - workload cluster" {
  run ck8s validate wc <<< $'y\n'

  assert_success
}

@test "configuration is invalid - kubespray:elastx:prod - service cluster" {
...
@test "configuration is invalid - kubespray:elastx:prod - workload cluster" {
...
@test "manifests are valid - kubespray:elastx:prod - service cluster" {
  run helmfile_template_kubeconform service_cluster

  assert_success
}

@test "manifests are valid - kubespray:elastx:prod - workload cluster" {

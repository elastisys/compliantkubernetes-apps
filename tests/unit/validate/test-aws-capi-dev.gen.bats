...
# bats file_tags=static,validate,aws,dev
...
@test "configuration is valid - aws:capi:dev - service cluster" {
  run ck8s validate sc <<< $'y\n'

  assert_success
}

@test "configuration is valid - aws:capi:dev - workload cluster" {
  run ck8s validate wc <<< $'y\n'

  assert_success
}

@test "configuration is invalid - aws:capi:dev - service cluster" {
...
@test "configuration is invalid - aws:capi:dev - workload cluster" {
...
@test "manifests are valid - aws:capi:dev - service cluster" {
  run helmfile_template_kubeconform service_cluster

  assert_success
}

@test "manifests are valid - aws:capi:dev - workload cluster" {

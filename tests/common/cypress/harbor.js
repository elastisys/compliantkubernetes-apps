Cypress.Commands.add('harborAdminLogin', (ingress) => {
  ...
  cy.contains('LOGIN VIA LOCAL DB').click()

  cy.yqSecrets('.harbor.password').then((password) => {
...
Cypress.Commands.add('harborStaticDexLogin', (ingress) => {
  ...
  cy.url().then((url) => {
    if (url.includes('oidc-onboard')) {
      cy.contains('label', 'Username').siblings().get('input').clear().type('dex-static-user')
...
Cypress.Commands.add('harborStaticDexPromote', (ingress) => {
  ...
  cy.get('button[id=set-admin]').then((element) => {
    if (element.text().includes('SET AS ADMIN')) {
      cy.wrap(element).click()
    }
  })

  cy.contains('admin@example.com').siblings().contains('Yes')
})

...
@test "static user has restricted access in namespace staging" {
  local expected="${BATS_TEST_DIRNAME}/resources/dev-access-list-staging"

  run kubectl auth whoami
  assert_output --partial "dev@example.com"

  run kubectl -n staging auth can-i --list --no-headers
  assert_success

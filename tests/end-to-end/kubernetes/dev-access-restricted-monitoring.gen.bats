...
@test "static user has restricted access in namespace monitoring" {
  local expected="${BATS_TEST_DIRNAME}/resources/dev-access-list-monitoring"

  run kubectl auth whoami
  assert_output --partial "dev@example.com"

  run kubectl -n monitoring auth can-i --list --no-headers
  assert_success

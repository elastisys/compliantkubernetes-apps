...
@test "static user has restricted access in namespace kube-system" {
  local expected="${BATS_TEST_DIRNAME}/resources/dev-access-list-kube-system"

  run kubectl auth whoami
  assert_output --partial "dev@example.com"

  run kubectl -n kube-system auth can-i --list --no-headers
  assert_success

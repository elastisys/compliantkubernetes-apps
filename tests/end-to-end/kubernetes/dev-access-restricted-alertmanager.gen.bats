...
@test "static user has restricted access in namespace alertmanager" {
  local expected="${BATS_TEST_DIRNAME}/resources/dev-access-list-alertmanager"

  run kubectl auth whoami
  assert_output --partial "dev@example.com"

  run kubectl -n alertmanager auth can-i --list --no-headers
  assert_success

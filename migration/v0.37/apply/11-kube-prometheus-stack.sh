...
run() {
  ...
  execute)
    ...
    if [[ "${CK8S_CLUSTER}" =~ ^(sc|both)$ ]]; then
      ...
      kubectl_do sc apply --server-side -f "${ROOT}"/helmfile.d/upstream/prometheus-community/kube-prometheus-stack/charts/crds/crds
    ...
    if [[ "${CK8S_CLUSTER}" =~ ^(wc|both)$ ]]; then
      ...
      kubectl_do wc apply --server-side -f "${ROOT}"/helmfile.d/upstream/prometheus-community/kube-prometheus-stack/charts/crds/crds

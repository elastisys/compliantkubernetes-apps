...
>
> - There are now safeguards in place to ensure `ck8s upgrade prepare` and `ck8s upgrade apply` are run in the correct order.<br>There's also a new `ck8s version` command that reports the verison of apps in the config and cluster(s).

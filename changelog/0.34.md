...
> - This PR changes the number of arguments required to run the `upgrade` command, as you now have to specify the specific cluster, `sc`, `wc`, or `both` to apply to both clusters as the command did previously. The ck8s usage has been updated to clarify this:Migration templating have also been updated to support and clarify these changes.

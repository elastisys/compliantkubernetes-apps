...
> - For UpCloud as a provider: The default configuration changed from using HostPorts on the Pods to NodePorts on the Service. This changes the used port numbers, so unless you reconfigure your load balancer to use the new NodePorts, ingress traffic will not be able to enter your environment. We have added a migration script for existing environments so that they will retain the same configuration as before the upgrade.
